	...
	this.paint = function (gr) {
		if (this.is_bio_panel) {
			if (this.bitmap.normal) {
				...
				_drawOverlay(gr, 0.14*panel.w, 0.1*panel.h, panel.w, panel.h, 180);
				_drawImage(gr, this.bitmap.normal, -1, 0, this.h, this.h, this.properties.aspect.value == image.full ? image.full_top_align : this.properties.aspect.value, 1.0, RGB(150, 150, 150));
			} else {
				_drawOverlay(gr, 0.14*panel.w, 0.1*panel.h, panel.w, panel.h);
			}
		} else if (this.bitmap.normal) {
			if (this.properties.aspect.value == image.full) {
				this.draw_blurred_image(gr);
				_drawImage(gr, this.bitmap.normal, -1, 0, this.h, this.h, this.properties.aspect.value, 1.0, RGB(150, 150, 150));
			} else {
				_drawImage(gr, this.bitmap.normal, -1, 0, this.h, this.h, this.properties.aspect.value);
			}
		}
	}
