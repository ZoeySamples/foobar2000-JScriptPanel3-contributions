function _images() {
	...
	this.draw_blurred_image = function (gr) {
		...
		_drawImage(gr, this.bitmap.heavyblur, 0, 0, panel.w, panel.h, image.crop, 0.6);
		//_drawImage(gr, this.bitmap.blur, 0, 0, panel.w, panel.h, image.crop, 0.2);
	}
	...
	this.update_image = function () {
		...
		if (this.image_index < this.image_paths.length) {
			...
			if (img) {
				...
				img.StackBlur(220);
				this.bitmap.heavyblur = img.CreateBitmap();
				img.Dispose();
			}
		}
	}
